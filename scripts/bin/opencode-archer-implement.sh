...
usage() {
  ...
  printf 'Reads the Archer handoff prompt from stdin.\n' >&2
}
...
if ! command -v archer >/dev/null 2>&1; then
...
mkdir -p "$PLAN_DIR"
PLAN_FILE="$(mktemp "$PLAN_DIR/archer-implement-plan.XXXXXX.md")"
cat > "$PLAN_FILE"
...
OPEN_SCRIPT="${OPENCODE_ARCHER_IMPLEMENT_OPEN_SCRIPT:-${HOME}/.config/opencode/bin/opencode-archer-implement-open}"
if [ ! -x "$OPEN_SCRIPT" ]; then
  printf 'Archer implement opener not found or not executable: %s\n' "$OPEN_SCRIPT" >&2
  exit 1
fi
